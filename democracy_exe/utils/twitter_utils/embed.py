...
def _add_optional_field(embed: discord.Embed, metadata: dict[str, Any], field: str, *, inline: bool = True) -> None:
    ...
    if field in metadata:
        embed.add_field(name=field.replace("_", " ").title(), value=metadata[field], inline=inline)
...
def _add_media_field(embed: discord.Embed, metadata: dict[str, Any]) -> None:
    ...
    if metadata.get("media_urls"):
        ...
        embed.add_field(name="Media", value=media_list, inline=False)
...
def create_tweet_embed(metadata: TweetMetadata) -> discord.Embed:
    ...
    _add_optional_field(embed, metadata, "url")

    if "content" in metadata:
        embed.description = metadata["content"]
...
def create_card_embed(metadata: TweetMetadata) -> discord.Embed:
    ...
    _add_optional_field(embed, metadata, "card_url", inline=False)
    _add_optional_field(embed, metadata, "card_description", inline=False)

    if "card_image" in metadata:
...
def create_info_embed(metadata: TweetMetadata) -> discord.Embed:
    ...
    _add_optional_field(embed, metadata, "url", inline=False)

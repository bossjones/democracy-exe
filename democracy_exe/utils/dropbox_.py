...
from democracy_exe.utils.aiodbx import aiosettings
...
def get_dropbox_client() -> dropbox.Dropbox | None: # pylint: disable=no-member
    dbx = None
    token=aiosettings.dropbox_cerebro_token.get_secret_value()
    try:
...
class AsyncDropBox:
    ...
    def __init__(self):
        self.dropbox_access_token = aiosettings.dropbox_cerebro_token.get_secret_value() # pylint: disable=no-member
